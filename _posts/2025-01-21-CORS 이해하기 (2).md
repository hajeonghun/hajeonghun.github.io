...
이를 확인해보려면, Chrome(크롬) 브라우저에서 개발자 도구의 `Disable cache` 옵션을 활성화(캐시 미사용) 시켜보면 요청 때 마다 사전요청(Preflight Request)이 전송되지만 비활성화(캐시 사용) 시키는 경우엔 첫 요청에만 사전요청이 전송되고 그 이후의 요청엔 실제 요청들만 전송되는 걸 인 할 수 있습니다.  
